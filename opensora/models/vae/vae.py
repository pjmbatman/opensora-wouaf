...
from customization_fin import customize_spatial_vae_decoder
...
class VideoAutoencoderKL(nn.Module):
    def __init__(
        ...
        # 수정1
        self.module = customize_spatial_vae_decoder(self.module, phi_dimension=48, lr_multiplier=1)
    ...
    def encode(self, x):
        ...
        return x
    # 수정
    def decode(self, x, enconded_fingerprint, **kwargs):
        # x: (B, C, T, H, W)
        B = x.shape[0]
        x = rearrange(x, "B C T H W -> (B T) C H W")
        if self.micro_batch_size is None:
            # 수정
            x = self.module.decode(x / self.scaling_factor, enconded_fingerprint).sample
        else:
            # NOTE: cannot be used for training
            bs = self.micro_batch_size
            x_out = []
            for i in range(0, x.shape[0], bs):
                x_bs = x[i : i + bs]
                # 수정
                x_bs = self.module.decode(x_bs / self.scaling_factor, enconded_fingerprint).sample
                x_out.append(x_bs)
            x = torch.cat(x_out, dim=0)
        x = rearrange(x, "(B T) C H W -> B C T H W", B=B)
        return x
...
class VideoAutoencoderPipeline(PreTrainedModel):
    ...
    def __init__(self, config: VideoAutoencoderPipelineConfig):
        ...
        # 수정1
        self.spatial_vae.module = customize_spatial_vae_decoder(self.spatial_vae.module, phi_dimension=48, lr_multiplier=1)
    ...
    # 수정2
    def decode(self, z, encoded_fingerprint, num_frames=None):
        ...
        if self.micro_frame_size is None:
            ...
            # 수정3
            x = self.spatial_vae.decode(x_z, encoded_fingerprint)
        else:
            x_z_list = []
    ...
    def forward(self, x, encoded_fingerprint):
        assert self.cal_loss, "This method is only available when cal_loss is True"
        z, posterior, x_z = self.encode(x)
        x_rec, x_z_rec = self.decode(z, encoded_fingerprint, num_frames=x_z.shape[2])
