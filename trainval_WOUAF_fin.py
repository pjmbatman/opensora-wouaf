...
def parse_args():
    ...
    type=int,
    default=2,
    help="num of results for displaying",
    )
    parser.add_argument(
    "--num_train_data",
    type=int,
    default=1000,
    help="num of dataset to train",
    )
    parser.add_argument(
    "--num_val_data",
    type=int,
    default=400,
    help="num of dataset to validate",
    )
...
"""def get_phis(phi_dimension, batch_size, eps=1e-8, seed=42):
    seed = int(time.time())  # 현재 시간을 기반으로 시드를 생성
    random.seed(seed)
    torch.manual_seed(seed)  # 시드를 고정
    phi_length = phi_dimension
    
    # 1 * 48 크기의 랜덤 마스크를 생성
    phi = torch.empty(1, phi_length).uniform_(0, 1)
    phi = torch.bernoulli(phi) + eps  # Bernoulli 분포로 0 또는 1 생성
    
    # 동일한 값을 배치 크기만큼 복제 (batch_size, phi_length) 형태로 확장
...
def decode_latents(vae, latents, enconded_fingerprint, frame):
    latents = 1 / 0.18215 * latents
    image = accelerator.unwrap_model(vae).decode(latents, enconded_fingerprint, frame)
    #image = accelerator.unwrap_model(vae).decode(latents)
...
# 수정
def get_params_optimize(vae_spatial, mapping_network, decoding_network):
    params_to_optimize = itertools.chain(vae_spatial.parameters(), mapping_network.parameters(), decoding_network.parameters())
...
def val(args, epoch, step, accelerator, weight_dtype, generation_scheduler, tokenizer, vae, mapping_network, decoding_network, test_dataloader, valid_aug, resize, metrics):
    ...
    with torch.no_grad():
        for val_step, batch in enumerate(test_dataloader):
            ...
            # .latent_dist.sample() 삭제
            latents = accelerator.unwrap_model(vae).encode(batch["pixel_values"].to(weight_dtype).to(accelerator.device))
            ...
            generated_image_latent_0 = generated_image_latent_0.squeeze(2)
            ...
            outputs = {'images': (generated_image_latent_0 / 2 + 0.5).clamp(0, 1), 'images_trainval': (generated_image_latent_0 / 2 + 0.5).clamp(0, 1)}
    ...
    gt_images = (batch["pixel_values"] / 2 + 0.5).clamp(0, 1)
    gt_images = gt_images.squeeze(2)

    combined_images = torch.cat((gt_images, generated_image_latent_0), dim=0)
    save_image(combined_images, '{0}/sample_images/sample_e_{1}_s_{2}.png'.format(args.output_dir, epoch, step), normalize=True, value_range=(0,1), scale_each=True)
    wandb.log({"Examples": wandb.Image(combined_images, caption="GT and Generated - sample_e_{0}_s_{1}".format(epoch, step))})

    """#Saving Image
    generated_image_latent_0 = (generated_image_latent_0 / 2 + 0.5).clamp(0, 1)
    save_image(generated_image_latent_0, '{0}/sample_images/sample_e_{1}_s_{2}.png'.format(args.output_dir,epoch,step),normalize=True, value_range=(0,1), scale_each=True)
    wandb.log({"Examples": wandb.Image(generated_image_latent_0, caption="sample_e_{0}_s_{1}".format(epoch, step))})
...
def main():
    ...
    vae_pipeline = VideoAutoencoderPipeline.from_pretrained("hpcai-tech/OpenSora-VAE-v1.2")
    ...
    # 수정2
    vae_pipeline.requires_grad_(False)
    vae_pipeline.spatial_vae.module.decoder.requires_grad_(True)
    # Temporal Vae는 customizing을 한게 없으니 굳이 학습 시킬 필요가 없음 -> Freeze
    #vae_pipeline.temporal_vae.decoder.requires_grad_(True)
    print(check_trainable_params(vae_pipeline))
    ...
    # 수정3
    # vae = customize_spatial_vae_decoder(vae, args.int_dimension, args.lr_mult)
    ...
    #수정 4
    vae_pipeline.to(accelerator.device, dtype=weight_dtype)
    mapping_network.to(accelerator.device, dtype=weight_dtype)
    decoding_network.to(accelerator.device, dtype=weight_dtype)

    optimizer = optimizer_cls(
        get_params_optimize(vae_pipeline, mapping_network, decoding_network),
    ...
    elif args.dataset_name == "webvid":
        isShuffle = False
        train_video_files, val_video_files = get_dataset_path()
        # 훈련 및 검증 데이터셋 생성
        train_dataset = create_dataset(train_video_files, args.num_train_data)
        print("train data loaded completed")
        val_dataset = create_dataset(val_video_files, args.num_val_data)
        print("validation data loaded completed")
        print(train_dataset)
        # 데이터셋을 DatasetDict로 묶기
        dataset = DatasetDict({
            "train": train_dataset,
            "test": val_dataset
        })
    ...
    def preprocess_train(examples):
        ...
        images = images.unsqueeze(2)  # (B, C, 1, H, W)로 차원 추가
    ...
    def preprocess_test(examples):
        images = [image.convert("RGB") for image in examples[image_column]]
        images = [test_transforms(image) for image in images]
    ...
    # 수정5
    vae_pipeline, mapping_network, decoding_network, optimizer, train_dataloader, test_dataloader, lr_scheduler = accelerator.prepare(
        vae_pipeline, mapping_network, decoding_network, optimizer, train_dataloader, test_dataloader, lr_scheduler
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(accelerator.unwrap_model(vae_pipeline), accelerator.accumulate(mapping_network), accelerator.accumulate(decoding_network)):
                # Convert images to latent space
                # 수정7
                #print(11111111111111111111)
                x = batch['pixel_values'].to(weight_dtype).to(accelerator.device)
                #print(x.size()) #torch.Size([4, 3, 1, 256, 256])
                #latents = accelerator.unwrap_model(vae_pipeline).encode(x)
                latents = accelerator.unwrap_model(vae_pipeline).encode(x)
                latents = latents * 0.18215
                #print(22222222)
                # Sample noise that we'll add to the latents
                ...
                encoded_phis = mapping_network(phis)
                frames = x.size(2)
                generated_image = decode_latents(vae_pipeline, latents, encoded_phis, frames)

                # 5차원을 다시 4차원으로
                batch['pixel_values'] = batch['pixel_values'].squeeze(2)
                generated_image = generated_image.squeeze(2)
                
                #리사이징
                generated_image = resize(generated_image)
                batch['pixel_values'] = resize((batch['pixel_values']))
                ...
                """print(generated_image.min().item(), generated_image.max().item())
                print(resize(batch['pixel_values']).min().item(), resize(batch['pixel_values']).max().item())"""

                loss_lpips_reg = loss_fn_vgg.forward(generated_image, batch['pixel_values']).mean()
                ...
                if accelerator.sync_gradients:
                    accelerator.clip_grad_norm_(get_params_optimize(accelerator.unwrap_model(vae_pipeline), mapping_network, decoding_network), args.max_grad_norm)
                optimizer.step()
                lr_scheduler.step()
                optimizer.zero_grad()

                """generated_image = (generated_image / 2 + 0.5).clamp(0, 1)
                gt_image = (batch['pixel_values'] / 2 + 0.5).clamp(0, 1)

                train_all_gt_images.append(gt_image)
                train_all_generated_images.append((generated_image / 2 + 0.5).clamp(0, 1))"""
        ...
        for step, batch in enumerate(test_dataloader):
            with torch.no_grad():
                ...
                # Collect images from the current batch
                generated_image = (generated_image / 2 + 0.5).clamp(0, 1)
                gt_image = (batch['pixel_values'] / 2 + 0.5).clamp(0, 1)

                val_all_gt_images.append(gt_image)
                val_all_generated_images.append(generated_image)

                if num == args.num_result:  # Modify this to include as many batches as needed
                    break

        # Concatenate the images along the width (dim=2) to align GT and generated images side by side
        combined_rows = [torch.cat((gt, gen), dim=2) for gt, gen in zip(val_all_gt_images, val_all_generated_images)]

        # Concatenate the rows along the height (dim=0) to stack batches vertically
        combined_images = torch.cat(combined_rows, dim=0)

        # Save the concatenated image
        save_image(combined_images, '{0}/sample_images/sample_e_{1}_s_{2}.png'.format(args.output_dir, epoch, step), normalize=True, value_range=(0,1), scale_each=True)

        val_acc = torch.mean(torch.tensor(list_val_bit_acc))
        print("Validation Acc: Bit-wise Acc in Epoch {0}: {1}".format(epoch, val_acc))
        wandb.log({"Validation Acc": val_acc.item(), "Validation Loss": val_loss / len(test_dataloader)})
    ...
    if accelerator.is_main_process:
        vae_pipeline = accelerator.unwrap_model(vae_pipeline)
        ...
        torch.save(vae_pipeline.state_dict(), args.output_dir + "/vae_pipeline.pth")
        torch.save(mapping_network.state_dict(), args.output_dir + "/mapping_network.pth")
        torch.save(decoding_network.state_dict(), args.output_dir + "/decoding_network.pth")
